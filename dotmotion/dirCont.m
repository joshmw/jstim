...
% task parameters
task{1}.segmin = [2 15]; task{1}.segmax = [2 15]; task{1}.getResponse = [0 0];
...
stimulus.transparent = 1; %nontransparency randomizes which dots belong with each motion component every frame update
...
stimulus.speed = 5; %initial response speed
...
task{1}.parameter.coherence1 = [1];
task{1}.parameter.coherence12 = [0]
task{1}.parameter.speed = [5];
...
if task.thistrial.thisseg == 2
    ...
    % set the coherence
    stimulus.dots = stimulus.dots.setCoherence(stimulus.dots,[task.thistrial.coherence1,task.thistrial.coherence12,0,0,0,0,0]);
    stimulus.dots2 = stimulus.dots2.setCoherence(stimulus.dots2,[task.thistrial.coherence2,0,0,0,0,0,0]);
    stimulus.dotsResponse = stimulus.dotsResponse.setCoherence(stimulus.dotsResponse,[1,0,0,0,0,0,0]);
    ...
    stimulus.dotsResponse = stimulus.dotsResponse.setDir(stimulus.dotsResponse,[task.thistrial.dir1,0,0,0,0,0,0]);
    
    % send initdots whether you want a transparent or nontransparent motion
    ...
    % init intial direction of response (confidence) and stimulus (current) - these change over presentation
    task.thistrial.confidence = 0;
    task.current = task.thistrial.dir1;
    task.current2 = task.thistrial.dir1;
...
elseif task.thistrial.thisseg == 1
    ...
    stimulus.dotsResponse = stimulus.dotsResponse.setCoherence(stimulus.dotsResponse,[0 0 0 0 0 0 0]);
...
if task.thistrial.thisseg == 1
    ...
    stimulus.dotsResponse = stimulus.dotsResponse.update(stimulus.dotsResponse);
    
    % draw the dots
    stimulus.dots = stimulus.dots.draw(stimulus.dots);
    stimulus.dotsResposne = stimulus.dotsResponse.draw(stimulus.dotsResponse)
...
elseif task.thistrial.thisseg == 2
    ...
    task.current = task.current+rand/5;
    task.current2 = task.current+10
    if task.current > 360;task.current=task.current-360;elseif task.current<0;task.current=task.current+360,end;
    if task.current2 > 360;task.current2=task.current2-360;elseif task.current2<0;task.current2=task.current2+360,end;
    % update the dots
    stimulus.dots = stimulus.dots.setDir(stimulus.dots,[task.current,task.current2,0,0,0,0,0]);
    stimulus.dots = stimulus.dots.update(stimulus.dots);
    stimulus.dots2 = stimulus.dots2.update(stimulus.dots2);
   ...
   if stimulus.estimateSpeed==1; stimulus.dotsResponse = stimulus.dotsResponse.setSpeed(stimulus.dotsResponse,task.thistrial.confidence); end;
   if stimulus.estimateDirection==1; stimulus.dotsResponse = stimulus.dotsResponse.setDir(stimulus.dotsResponse,[task.thistrial.confidence,0,0,0,0,0,0]); end;
...
EccR = strcat('xCenter=',num2str(-stimulus.Ecc1));
...
stimulus.dotsResponse = initJdots('framesPerSecond',myscreen.framesPerSecond,width,contrast,speed,EccR,dotDense,movshon,randwalk);
